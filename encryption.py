...
def store_private_key_file(private_key,password: bytes):
    pem = private_key.private_bytes(encoding=serialization.Encoding.PEM, format=serialization.PrivateFormat.PKCS8, encryption_algorithm=serialization.BestAvailableEncryption(password))
...
def store_public_key_file(public_key):
    pem = public_key.private_bytes(encoding=serialization.Encoding.PEM, format=serialization.PublicFormat.SubjectPublicKeyInfo)
...
def read_private_key(private_key_file,password_input:bytes ):
    with open(private_key_file, "rb") as key_file:
        private_key = serialization.load_pem_private_key(key_file.read(), password=password_input, backend=default_backend())
...
def encrypt(input_string: bytes, public_key):
    encrypted_message = public_key.encrypt(input_string,padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None))
...
def decrypt(encrypted_message, private_key):
    original_message = private_key.decrypt(encrypted_message,padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None))
